...
if (isset($_POST['update_order'])) {
   ...
   $order_update_id = $_POST['order_id'];<?php
   ...
   if (isset($_POST['update_order'])) {
      ...
      $date = date("d.m.Y");
      $stmt = $conn->prepare("UPDATE `confirm_order` SET payment_status = ?, date = ? WHERE order_id = ?");
      $stmt->execute([$update_payment, $date, $order_update_id]);
      $message[] = 'payment status has been updated!';
   }
   
   if (isset($_GET['delete'])) {
      $delete_id = $_GET['delete'];
      $stmt = $conn->prepare("DELETE FROM `confirm_order` WHERE order_id = ?");
   ...
   <body>
      ...
      <section class="placed-orders">
         ...
         <div class="box-container">
            <?php
            $stmt = $conn->query("SELECT * FROM `confirm_order`") or die('query failed');
            if ($stmt->rowCount() > 0) {
